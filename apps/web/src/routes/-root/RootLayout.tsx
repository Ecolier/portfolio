...
export default function RootLayout() {
  ...
  const currentHref = useLocation({
    select: (location) => location.publicHref,
  });
  ...
  const { targetLocale, switchHref } = useLangSwitch(locale, currentHref);
          ...
          {navLinks.map((link, index) => (
            <Fragment key={link.to}>
              <div>
                <Link
                  {...link}
                  className={`type-ui ${isActivePath(currentHref, link.to) ? "text-foreground" : "text-subtle-foreground"}`}
                >
                  {link.label}
                </Link>
              </div>
